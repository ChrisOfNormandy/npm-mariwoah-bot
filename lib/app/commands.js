...
function getCommandList() {
    let list = [];
    list.push(new Command('general', 'help', (data) => help(data, commands.getList()))
        ...
        .setSetting('commandClear', { delay: 0 }), new Command('general', 'say', say)
        .setRegex(/(say)/, /\s(.+)/, [1]), new Command('general', 'print', (data) => new Output(data.content.replace(data.bot.prefix + data.command, '').trim()).setValues(data.content.replace(data.bot.prefix + data.command, '').trim()).resolve())
        .setRegex(/(print)/));
