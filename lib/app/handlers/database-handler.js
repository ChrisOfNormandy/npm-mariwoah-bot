...
function search(table, options = {}) {
    ...
    return query(`select * from ${table}` + opts.join(' and '));
}
/**
 *
 * @param {string} table
...
function deleteTablesFromList(list, index, drop = false) {
    const table = list[index];
    console.log(drop
        ? 'Dropping table: ' + table.name
        : 'Deleting entries from table: ' + table.name);
    return new Promise((resolve, reject) => {
        exists(table)
            .then((has) => {
            if (!has)
                ...
                query(`delete from ${table.name}`)
                    .then(() => {
                    if (!drop)
                        deleteTablesFromList(list, index, true)
                            .then((r) => resolve(r))
                            .catch((err) => reject(err));
